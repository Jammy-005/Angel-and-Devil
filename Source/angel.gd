...
func compute_tiles_in_range(dist, generate):
	if FORCE_EUCLIDEAN or ADJACENT == 2 or ADJACENT == 6:
		return _compute_tiles_in_range(int(floor(dist)), generate)
	elif ADJACENT == 4:
		return _compute_tiles_in_chessboard(dist, generate)
	elif ADJACENT == 3:
		return _compute_tiles_in_triangle(int(floor(dist)), generate)
